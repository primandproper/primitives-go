...
func TestRegister_failingObservabilityIsAnError(T *testing.T) {
	...
	T.Run("RegisterMessageQueue", func(t *testing.T) {
		t.Parallel()
		...
		_, err := do.Invoke[messagequeue.ConsumerProvider](i)
		must.Error(t, err)
		test.ErrorIs(t, err, errBuild)
	})

	T.Run("RegisterMessageQueue (PublisherProvider)", func(t *testing.T) {
		t.Parallel()
		...
		_, err := do.Invoke[messagequeue.PublisherProvider](i)
		must.Error(t, err)
		test.ErrorIs(t, err, errBuild)
	})
}
