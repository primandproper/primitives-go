...
// WithRouterOptions passes opts to the Router that NewRouter layers on the
// backend, applied after the options derived from configuration. NewBackend
// ignores them.
//
// They cannot be a second variadic on NewRouter: Go allows one per function,
// and that slot is what makes the observability optional.
func WithRouterOptions(opts ...routing.RouterOption) Option {
	return func(o *options) { o.router = append(o.router, opts...) }
}
