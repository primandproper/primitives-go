...
// Config configures the httprouter (julienschmidt/httprouter) router backend.
// Its fields mirror the other backends so a service can switch backends without
// changing its configuration shape.
type Config struct {
	_ struct{} `json:"-" yaml:"-"`
