...
// WithManagerOptions passes opts to the Manager, which applies them after the
// options it derives from configuration — so a caller can override anything.
//
// They cannot be a second variadic on NewManager: Go allows one per function,
