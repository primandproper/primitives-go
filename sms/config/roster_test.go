...
// rosterLine matches one entry of the indented provider block in sms's package
// doc: a tab, the provider's name, and the prose describing it.
var rosterLine = regexp.MustCompile(`^\t([a-z0-9]+) {2,}\S`)

// TestDocRosterMatchesProviders checks the provider list in sms's package doc
// against the roster this package dispatches on.
//
// The two are a copy of each other, and a copy of a list is the kind of
// documentation that goes quietly wrong: a vendor is added here and the doc
// goes on naming only the ones that were there before, with nothing saying so
// until somebody reads both. emailcfg's copy of this test was written after
// exactly that drift; this roster is short today, which is the cheapest moment
// to start guarding it.
//
// It lives in this package rather than in sms because providers is
...
func TestDocRosterMatchesProviders(t *testing.T) {
	...
	test.Eq(t, expected, documented, test.Sprintf(
		"sms/doc.go names %v; smscfg dispatches on %v", documented, expected,
	))
}
