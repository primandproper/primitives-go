...
type options struct {
	...
	pool      []jobs.PoolOption
	scheduler []jobs.SchedulerOption
}
...
// WithPoolOptions passes opts to NewPool, which applies them after the options it
// derives from configuration — so a caller can override anything. The other
// constructors ignore them.
func WithPoolOptions(opts ...jobs.PoolOption) Option {
	return func(o *options) { o.pool = append(o.pool, opts...) }
}

// WithSchedulerOptions passes opts to NewScheduler, which applies them after the options it
// derives from configuration — so a caller can override anything. The other
// constructors ignore them.
func WithSchedulerOptions(opts ...jobs.SchedulerOption) Option {
	return func(o *options) { o.scheduler = append(o.scheduler, opts...) }
}
