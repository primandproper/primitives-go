...
func TestNewRedisRateLimiter(T *testing.T) {
	...
	T.Run("with no addresses", func(t *testing.T) {
		...
		rl, err := NewRedisRateLimiter(t.Context(), Config{}, 10, 20)
		test.Error(t, err)
		test.Nil(t, rl)
	})
	...
	T.Run("with single address", func(t *testing.T) {
		...
		rl, err := NewRedisRateLimiter(t.Context(), cfg, 10, 20)
		test.NoError(t, err)
		test.NotNil(t, rl)
	})
	...
	T.Run("with error creating allowed counter", func(t *testing.T) {
		...
		rl, err := NewRedisRateLimiter(t.Context(), cfg, 10, 20, WithMetricsProvider(mp))
		test.Error(t, err)
		test.Nil(t, rl)
