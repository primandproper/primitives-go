...
There are three emit methods and a set of With* methods that derive a logger
carrying more context. Error takes what was being attempted alongside the error,
because "what failed" without "what was being tried" is the message that turns up
in a search and answers nothing. There are no formatting variants: a message is a
...
WarnLevel is a threshold with no method behind it: nothing can emit at warn, so
configuring it means Info and Debug are dropped and only Error survives. It is
the level to choose when that is what is wanted, and not the one to reach for
expecting warnings.
*/
package logging
