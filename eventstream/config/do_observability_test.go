...
func TestRegister_failingObservabilityIsAnError(T *testing.T) {
	...
	T.Run("RegisterEventStreamUpgrader", func(t *testing.T) {
		t.Parallel()
		...
		_, err := do.Invoke[eventstream.EventStreamUpgrader](i)
		must.Error(t, err)
		test.ErrorIs(t, err, errBuild)
	})

	T.Run("RegisterBidirectionalEventStreamUpgrader", func(t *testing.T) {
		t.Parallel()
		...
		_, err := do.Invoke[eventstream.BidirectionalEventStreamUpgrader](i)
		must.Error(t, err)
		test.ErrorIs(t, err, errBuild)
	})
}
