...
import (
	...
	"github.com/primandproper/primitives-go/v2/featureflags/launchdarkly"
	"github.com/primandproper/primitives-go/v2/featureflags/posthog"
...
func TestConfig_NewFeatureFlagManager(T *testing.T) {
	...
	T.Run("with default/noop provider", func(t *testing.T) {
		...
		ffm, err := cfg.NewFeatureFlagManager(t.Context(), http.DefaultClient, cbnoop.NewCircuitBreaker())
		must.NoError(t, err)
		must.NotNil(t, ffm)
	})
...
func TestNewFeatureFlagManager(T *testing.T) {
	T.Run("with noop provider", func(t *testing.T) {
		...
		ffm, err := NewFeatureFlagManager(ctx, cfg, http.DefaultClient)
		must.NoError(t, err)
		must.NotNil(t, ffm)
	})
	...
	T.Run("with circuit breaker error", func(t *testing.T) {
		...
		ffm, err := NewFeatureFlagManager(ctx, cfg, http.DefaultClient, WithMetricsProvider(mp))
		must.Error(t, err)
		must.Nil(t, ffm)
