...
func TestWithConfigModifiers(T *testing.T) {
	...
	T.Run("accumulates across calls and applies in order", func(t *testing.T) {
		...
		o := newOptions([]Option{
			WithConfigModifiers(func(c ld.Config) ld.Config { applied = append(applied, "first"); return c }),
			WithConfigModifiers(
				func(c ld.Config) ld.Config { applied = append(applied, "second"); return c },
				func(c ld.Config) ld.Config { applied = append(applied, "third"); return c },
			),
		})
